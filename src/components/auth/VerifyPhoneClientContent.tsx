// src/components/auth/VerifyPhoneClientContent.tsx
'use client';
...
export default function VerifyPhoneClientContent() {
  ...
  useEffect(() => {
    ...
    if (userIdFromQuery) {
      ...
      if (phoneEndingFromQuery) {
        ...
        // If userId is from query but no phoneEnding, try to get it
        // This might happen if a user bookmarks the page without phoneEnding
        ...
        try {
          ...
          if (user.id && !user.phone_verified) {
            ...
            if (user.phone_number) {
              ...
              // User has no phone number registered, which is an issue for OTP.
              // Attempt resend to trigger any server-side checks or at least get an error message if appropriate.
    ...
    if (result.success) {
      ...
      if (userJson) {
          try {
              ...
              if (user.id === userId) { // Make sure we're updating the correct user's session
                  localStorage.setItem('loggedInUser', JSON.stringify({...user, phone_verified: true }));
                  window.dispatchEvent(new CustomEvent('userSessionChanged')); // Notify other parts of the app
              }
  ...
  if (!userId) { // This case should ideally be caught by the error state above if no user can be determined
     return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
            <AlertTriangle className="h-16 w-16 text-destructive mb-4" />
