...
export default function PropertyComments({ propertyId, propertySlug }: PropertyCommentsProps) {
  ...
  const handleCommentAdded = (newComment: CommentType) => {
    // Add new comment (parent or reply) to the top of the list to trigger re-memoization
  ...
  const handleAddTopLevelComment = async () => {
    ...
    startSubmittingTransition(async () => {
      ...
      if (result.success && result.comment) {
        ...
        handleCommentAdded(result.comment); // Use unified handler
  ...
  const commentTree = useMemo(() => {
    ...
    // Sort root comments by most recent first
