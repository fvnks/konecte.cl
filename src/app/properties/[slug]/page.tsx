...
import { MapPin, BedDouble, Bath, HomeIcon, Tag, AlertTriangle, UserCircle, DollarSign, ParkingCircle, Trees, CheckSquare, MessageSquare, CalendarDays } from "lucide-react";
...
export default async function PropertyDetailPage({ params }: { params: { slug: string } }) {
  ...
  const featureIcons: Record<string, React.ElementType> = {
    ...
    "Piscina": Trees, // No hay un icono específico para piscina, Trees puede evocar aire libre.
    "Bodega": HomeIcon, // Usar HomeIcon o similar para almacenamiento
    "Gimnasio": CheckSquare, // CheckSquare o algo genérico de amenidad
        ...
        <CardContent className="p-6 md:p-8 pt-0 space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 text-center">
            {[
              ...
              { icon: HomeIcon, label: `${property.areaSqMeters} m²` },
          ...
          {property.author && (
            <div className="border-t pt-6 mt-8">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 bg-secondary/30 p-4 rounded-lg">
                ...
                <Button variant="default" size="lg" className="w-full sm:w-auto mt-2 sm:mt-0 rounded-md">
                  <MessageSquare className="mr-2 h-5 w-5"/> Contactar Anunciante
                </Button>
              </div>
            </div>
          )}
